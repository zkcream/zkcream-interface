...
import { useMaciContract } from '../hooks/useContract'

export default async function verify (data: any) {
  ...
  const expectedResultsCommitment = genTallyResultCommitment(tally, salt, resultsCommitment)
  ...
  if (onChainTvcCommitment.toString() === expectedTvcCommitment.toString()) {
    ...
    throw new Error('Error: the total spent voice credit commitment in the MACI contract does not match the expected commitment')
  }
  ...
  if (onChainPvcCommitment.toString() === expectedPvcCommitment.toString()) {
    ...
    throw new Error('Error: the per vote option spent voice credit commitment in the MACI contract does not match the expected commitment')
  }
  ...
  if (onChainTotalVotes.toString() === expectedTotalVotes.toString()) {
    ...
    throw new Error('Error: the total votes value in the MACI contract does not match the expected sum of the vote tally')
  }
}